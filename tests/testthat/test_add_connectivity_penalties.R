...
test_that('minimum set objective (binary decisions)', {
  ...
  c_data <- Matrix::sparseMatrix(i=c_data@i[c_data@x!=0], 
    j=c_data@j[c_data@x!=0], x=c_data@x[c_data@x!=0], giveCsparse=FALSE, 
    index1=FALSE, dims=c(n_pu, n_pu))
  ...
  for (pos in seq_along(c_data@i)) {
    ...
    connection_columns_for_i <- sapply(rows_i, function(r) which(o$A()[r,]==1))
    connection_columns_for_j <- sapply(rows_j, function(r) which(o$A()[r,]==1))
...
test_that('maximum representation objective (binary decisions)', {
  ## make data
  data(sim_pu_raster, sim_features)
  c_matrix <- boundary_matrix(sim_pu_raster)
  class(c_matrix) <- 'dgCMatrix'
