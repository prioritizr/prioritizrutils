...
rasterToLines <- function(x) {
  ...
  pts$vertical_start_y <- pts$y + yres
  pts$vertical_end_x <- pts$x
  pts$vertical_end_y <- pts$y - yres
  pts$horizontal_start_x <- pts$x + xres
  pts$horizontal_start_y <- pts$y
  pts$horizontal_end_x <- pts$x - xres
...
locked_in_cells <- sample(cells, size=floor(raster::ncell(sim_landscape)*0.1))
locked_out_cells <- sample(cells, size=floor(raster::ncell(sim_landscape)*0.1))
